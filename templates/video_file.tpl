...
function addText(commentBody) {
  if(commentBody.value == '') {
    commentBody.value = '{/literal}{$video_file20}{literal}';
...
function checkText() {
  ...
  commentSubmit.value = '{/literal}{$video_file14}{literal}';
...
function addComment(is_error, comment_body, comment_date) {
  if(is_error == 1) {
    ...
    if(comment_body == '') {
      commentError.innerHTML = '{/literal}{$video_file15}{literal}';
    } else {
      commentError.innerHTML = '{/literal}{$video_file16}{literal}';
    }
    var commentSubmit = document.getElementById('comment_submit');
    commentSubmit.value = '{/literal}{$video_file11}{literal}';
    ...
    {/literal}
      {if $user->user_info.user_id != 0}
        newTable += "<a href='{$url->url_create('profile',$user->user_info.user_username)}'><img src='{$user->user_photo('./images/nophoto.gif')}' class='photo' border='0' width='{$misc->photo_size($user->user_photo('./images/nophoto.gif'),'75','75','w')}'></a></td><td class='album_item2'><table cellpadding='0' cellspacing='0' width='100%'><tr><td class='album_comment_author'><b><a href='{$url->url_create('profile',$user->user_info.user_username)}'>{$user->user_info.user_username}</a></b> - {$datetime->cdate("`$setting.setting_timeformat` `$video_file22` `$setting.setting_dateformat`", $datetime->timezone($smarty.now, $global_timezone))}</td><td class='album_comment_author' align='right' nowrap='nowrap'>&nbsp;[ <a href='user_messages_new.php?to={$user->user_info.user_username}'>{$video_file23}</a> ]</td>";
      {else}
        newTable += "<img src='./images/nophoto.gif' class='photo' border='0' width='75'></td><td class='album_item2'><table cellpadding='0' cellspacing='0' width='100%'><tr><td class='album_comment_author'><b>{$video_file19}</b> - {$datetime->cdate("`$setting.setting_timeformat` `$video_file22` `$setting.setting_dateformat`", $datetime->timezone($smarty.now, $global_timezone))}</td><td class='album_comment_author' align='right' nowrap='nowrap'>&nbsp;</td>";
      {/if}
...
{* SET ALBUM TITLE *}
{if $album_info.album_title == ""}{assign var="album_title" value=$video_file3}{else}{assign var="album_title" value=$album_info.album_title}{/if}

<div class='page_header'><a href='{$url->url_create('profile', $owner->user_info.user_username)}'>{$owner->user_info.user_username}</a>{$video_file4} <a href='{$url->url_create('video_albums', $owner->user_info.user_username)}'>{$video_file5}</a> &#187; <a href='{$url->url_create('video', $owner->user_info.user_username, $album_info.album_id)}'>{$album_title}</a></div>
...
<td align='center' nowrap='nowrap' style='padding-right: 8px;'><b>[ <a href='{$url->url_create('video', $owner->user_info.user_username, $album_info.album_id)}'>{$video_file9}{$owner->user_info.user_username}{$video_file10}</a> ]</b></td>
...
<td align='center'>
  ...
  {if $media_info.media_code != ""}{$media_info.media_code|stripslashes|htmlspecialchars_decode}<br /><br />{/if}
  ...
  <td>
    ...
    <tr><td class='button'>
      <a href='user_report.php?return_url={$url->url_current()}'><img src='./images/icons/report16.gif' border='0' class='icon'>{$video_file12}</a>
    </td></tr>
    </table>
  </td>
  </tr>
...
<tr>
<td class='header'>
  {$video_file13} (<span id='total_comments'>{$total_comments}</span>)
</td>
</tr>
{if $allowed_to_comment != 0}
  <tr>
  <td class='album_postcomment'>
    <form action='video_file.php' method='post' target='AddCommentWindow' onSubmit='checkText()'>
    <textarea name='comment_body' id='comment_body' rows='2' cols='65' onfocus='removeText(this)' onblur='addText(this)' style='color: #888888; width: 100%;'>{$video_file20}</textarea>
    ...
    {if $setting.setting_comment_code == 1}
      ...
      <td width='10'><img src='./images/icons/tip.gif' border='0' class='icon' onMouseover="tip('{$video_file21}')"; onMouseout="hidetip()"></td>
    {/if}
    <td align='right' style='padding-top: 5px;'>
    <input type='submit' id='comment_submit' class='button' value='{$video_file11}'>
...
<td class='album' id='media_comments'>
  ...
  {section name=comment_loop loop=$comments}
    ...
    <td class='album_item2'>
      <table cellpadding='0' cellspacing='0' width='100%'>
      <tr>
      <td class='album_comment_author'><b>{if $comments[comment_loop].comment_author->user_info.user_id != 0}<a href='{$url->url_create('profile',$comments[comment_loop].comment_author->user_info.user_username)}'>{$comments[comment_loop].comment_author->user_info.user_username}</a>{else}{$video_file19}{/if}</b> - {$datetime->cdate("`$setting.setting_timeformat` `$video_file22` `$setting.setting_dateformat`", $datetime->timezone($comments[comment_loop].comment_date, $global_timezone))}</td>
      <td class='album_comment_author' align='right' nowrap='nowrap'>&nbsp;[ <a href='user_messages_new.php?to={$comments[comment_loop].comment_author->user_info.user_username}'>{$video_file23}</a> ]</td>
      </tr>
      <tr>
