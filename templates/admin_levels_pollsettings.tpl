...
{* $Id: admin_levels_pollsettings.tpl 16 2009-01-13 04:01:31Z john $ *}
...
<td valign='top' class='pagecell' rowspan='{math equation="x+5" x=$level_menu|@count}'>
  ...
  <tr>
    <td class='setting2'>
      ...
      <tr>
        <td><input type='radio' name='level_poll_allow' id='poll_allow_1' value='1'{if  $poll_allow} CHECKED{/if}>&nbsp;</td>
        <td><label for='poll_allow_1'>{lang_print id=2500009}</label></td>
      </tr>
      <tr>
        <td><input type='radio' name='level_poll_allow' id='poll_allow_0' value='0'{if !$poll_allow} CHECKED{/if}>&nbsp;</td>
        <td><label for='poll_allow_0'>{lang_print id=2500010}</label></td>
      </tr>
      </table>
    </td>
  </tr>
