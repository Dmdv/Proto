...
На этой странице вы можите управлять (добавлять, удалять, изменять, устанавливать цену) подарками в вашей социальной сети<BR>

<a href="javascript:;" onclick="return hs.htmlExpand(this,{literal} { contentId: 'add-gifts' }{/literal} )" class="highslide">Добавить подарок</a>
<a href="javascript:;" onclick="return hs.htmlExpand(this,{literal} { contentId: 'add-gifts-cat' } {/literal})" class="highslide">Добавить категорию </a>
...
{if $total_gifts == 0}
  ...
  <tr>
  <td align='center'>
    <div class='box' style='width: 300px;'><b>1</b></div>
  </td>
  </tr>
  </table>
  <br>
...
{else}
  ...
  <div class='pages'>{$total_gifts} Найденные подарки &nbsp;|&nbsp; страницы {section name=page_loop loop=$pages}{if $pages[page_loop].link == '1'}{$pages[page_loop].page}{else}<a href='admin_gifts.php?s={$s}&p={$pages[page_loop].page}&f_title={$f_title}&f_owner={$f_owner}'>{$pages[page_loop].page}</a>{/if} {/section}</div>
<table width=100%><tr><td>
  <table cellpadding='0' cellspacing='0' class='list'>
  <tr>
  <td class='header' colspan='5' align='center'>управление подарками</td>
  </tr>
  <tr>
  <td class='header' ><a class='header' href='admin_gifts.php?s={$i}&p={$p}'>ID</a></td>
  <td class='header' align='center'>Категория</td>
  <td class='header' align='center'><a class='header' href='admin_gifts.php?s={$su}&p={$p}'>Цена</a></td>
  <td class='header' >Опции</td>
  </tr>
  {section name=gifts_loop loop=$gifts}
    ...
    <td class='item'>[ <a href='../images/gifts/{$gifts[gifts_loop].gifts_id}.png' class="highslide" onclick="return hs.expand(this)">просмотр</a> ] [ <a href='javascript:;'  onclick="return hs.htmlExpand(this,{literal}{{/literal}contentId: 'gifts_{$gifts[gifts_loop].gifts_id}' {literal}}{/literal} )" class="highslide">изменить</a> ] [ <a href='admin_gifts.php?task=deletegifts&gifts_id={$gifts[gifts_loop].gifts_id}&s={$s}&p={$p}'>удалить</a> ]
...
<input type='hidden' name='task' value='edit_gifts'>
  ...
  <td align='center' valign='top' colspan=4>
    <div class='pages2'>{$total_gifts} найдено подарков &nbsp;|&nbsp; страницы {section name=page_loop loop=$pages}{if $pages[page_loop].link == '1'}{$pages[page_loop].page}{else}<a href='admin_gifts.php?s={$s}&p={$pages[page_loop].page}&f_title={$f_title}&f_owner={$f_owner}'>{$pages[page_loop].page}</a>{/if} {/section}</div>
  </td>
  </tr>
  </table>
{/if}
